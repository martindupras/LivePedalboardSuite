...
insertAtWithName { | argName = "passthrough", index |
    ...
    functionToInsert = procLib.get(argName.asSymbol); // seems to work
    if (functionToInsert.isNil) {
        ("NChain insertAt: no processor function named '" ++ argName ++ "' found in library; skipping").postln;
        ^this;
    };
    postln("NChain insertAt: got function from library for '" ++ argName ++ "'");
    ...
    insertIndex = index.clip(0, chainList.size); // clamp to valid range
    chainList = chainList.copy.insert(insertIndex, receivedNdefName);

    this.rewireChain;
    ("NChain insertAt: added " ++ receivedNdefName ++ " at index " ++ insertIndex).postln;
    ^this;
}


	// OLDER INSERT AT METHOD; KEEP FOR REFERENCE for now
