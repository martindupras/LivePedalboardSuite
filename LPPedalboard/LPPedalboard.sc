...
//// </sanity checks>
        ...
        testInputSignalIsOn = true; // turn on test signal for now
        this.testInputSignalBeOn;    // will wire via testInputSignalSwitch
    ...
    // added in v1.1.13
    testInputSignalSwitchOLDER{ | testInputSignalIsOn|
    ...
    // added in v1.1.13 - better version
    testInputSignalSwitch {
        var chosenSourceSym;

        // Ensure a simple built-in test source exists (rename as you prefer)
        Ndef(\testpink, {
            var sig;
            // stereo-friendly, level-safe test source
            sig = WhiteNoise.ar(0.1 ! numChannels) * SinOsc.kr(1).range(0, 1);
            sig
        });

        // Choose which source to feed into pedalboardIn
        chosenSourceSym = if(testInputSignalIsOn) { \testpink } { defaultSource.asSymbol };

        // Route: pedalboardIn <<> chosen source
        Ndef(pedalboardInSym.asSymbol) <<> Ndef(chosenSourceSym);
    ...
    // added in v1.1.13
   // testInputSignalBeOn { this.testInputSignalIsOn = true; postln("Test signal ON"); ^this }
   // testInputSignalBeOff { this.testInputSignalIsOn = false; postln("Test signal OFF"); ^this }

// better versions:
// added in v1.1.13  — corrected and unified
    testInputSignalBeOn {
        this.testInputSignalIsOn_(true);
        this.testInputSignalSwitch;
        "Test signal ON".postln;
        ^this
    }

    testInputSignalBeOff {
        this.testInputSignalIsOn_(false);
        this.testInputSignalSwitch;
        "Test signal OFF".postln;
        ^this
    }
