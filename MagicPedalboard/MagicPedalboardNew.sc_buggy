...
- Initializes two sink proxies (\chainA, \chainB), materializes stereo buses, and enforces an
  “exclusive current” invariant (Option A) so only CURRENT is audible while NEXT remains prebuilt and silent.
...
- Switching uses switchChain(fadeTime=0.1) with a short crossfade and pointer swap (CURRENT ↔ NEXT).
...
/* OLD rebuildUnbound { arg listRef;
 ...
 Ndef(leftKey) <<> Ndef(rightKey);
    ...
    *help {
        var text;
        text = "
MagicPedalboardNew.help  —  quick guide
